...
interface skillsDataTypes{skillName:string,developers:string[],profile:{technologies:string[],roles:string[]}}
const App = () => {
...
const [ShowAddForm,setShowAddForm]=useState(false)
 ...
 const currentSkillsData = skillsData
 currentSkillsData.push(skillData)
 ...
 function OnChange(e:any,fieldName:string) {
   ...
   if (fieldName ==="technologies"|| fieldName ==="roles"){
     setSkillData({...skillData,profile:{...skillData.profile,[fieldName]:[e.target.value]}})
   } else if (fieldName ==="developers"){setSkillData({...skillData,developers:[e.target.value]})}
     else {{setSkillData({...skillData,[fieldName]:e.target.value})}
 }
}
...
setShowAddForm(true)
  ...
  return (
     ...
     <a id="search-button" className="button1" onClick={() => loadSkill()}>Search</a>
       ...
       <a id="add-skill-button" className="button1" onClick={HandleClickAddBtn}>Add Skills</a>
         {showSkill? name : ''}
         {showSkills? listSkills: ''}</>):(<form onSubmit= {OnSubmit}> 
          <body>
          <h5> Skill Name </h5>
          <input type="text" id="skillName" name="skillName" onChange= {(e)=>OnChange(e,"skillName")}style={{width:"200px", height:"40px"}}/>
          <h5> Developers </h5>
          <select id="developers" name="developers" onChange= {(e)=>OnChange(e,"developers")}style={{width:"200px", height:"40px"}}>
          ...
          <h5> Thecnologies </h5>
          <select id="technologies" name="technologies" onChange= {(e)=>OnChange(e,"technologies")}style={{width:"200px", height:"40px"}}>
            <option value="Java">
            Java
            </option>
            <option value="React">
            ...
            <h5> Roles </h5>
            <select id="roles" name="roles" onChange= {(e)=>OnChange(e,"roles")}style={{width:"200px", height:"40px"}}>
            ...
            <option value="UI/UX_Designer">
            UI/UX Designer
            </option>
            </select>
            </body>
          <button type="submit">Add Skills </button></form>)
       }
